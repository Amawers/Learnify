...
<div class="flex flex-wrap">
    ...
    <div class="card w-96 bg-base-100 shadow-xl mb-10 ml-4">
        <div class="flex items-center mt-3 -mb-5 z-10">
            <x-instructor-image />
            <x-instructor-name :name="__($data['name'])"/>
        </div>
        <figure class="px-10 ">
            <x-course-image />
        </figure>
        <div class="card-body items-center text-center">
          <div class="flex items-center justify-between -mt-12 w-72">
            <span class="card-title text-xs">{{ $data["course"] }}</span>
            <x-course-progress :value="$data['value']"/>
          </div>
          <x-completion-percentage :value="$data['value']"/>
